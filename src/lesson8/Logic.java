...
public class Logic {
    ...
    private static final JFrame jFrame = new JFrame("Игра окончена");


    static void go() {
        gameFinished = true;
        if (checkWinLines(DOT_X, DOTS_TO_WIN)) {
            JOptionPane.showMessageDialog(jFrame, "Поздравляем! Вы выиграли!");
            return;
        }
        if (isFull()) {
        ...
        if (checkWinLines(DOT_O, DOTS_TO_WIN)) {
            JOptionPane.showMessageDialog(jFrame, "Компьютер победил.");
            return;
        }
        if (isFull()) {
            JOptionPane.showMessageDialog(jFrame, "Ничья");
